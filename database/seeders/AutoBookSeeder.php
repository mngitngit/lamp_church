    ...
    public function run()
    {
        ...
        foreach ($bookMe as $data) {
            ...
            if (!$is_booking_bypassed) {
                ...
                $registration = Registration::where('uuid', $data['uuid'])->where('attending_option', AttendingOption::Online)->first();

                if ($registration) {
                    $registration->update(['attending_option' => AttendingOption::Hybrid, 'can_book' => true]);
                }
